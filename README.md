...
optional capture hooks, protocol 1.28.0, SessionStart injection, and official
Claude Code capture hooks).

The native **Codex** package under `plugins/teamshared/` uses the server's MCP
OAuth discovery flow and treats Codex as a first-class client: protocol 1.28.0,
...
| `claude/` | Claude Code plugin (remote MCP via native `/mcp` OAuth + protocol 1.28.0 + capture hooks needing `TEAMSHARED_TOKEN`) |
...
| `plugins/teamshared/` | Native Codex plugin (OAuth MCP + protocol 1.28.0 + official capture hooks) |
...
Unbound `/mcp` keeps working. `SessionStart` injects protocol 1.28.0;
...
`SessionStart` injects protocol 1.28.0; `$status` checks health + version.
...
The Claude Code package ships protocol **1.28.0** (`teamshared-memory`),
`/teamshared:status`, and official Claude Code capture hooks. The Codex
package ships the same 1.28.0 loop plus official Codex hooks (`SessionStart`,
