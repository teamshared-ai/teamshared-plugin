...
Codex as a first-class TeamShared client: recall-first protocol **1.29.0**,
...
2. Start a new task so `SessionStart` injects protocol 1.29.0.
3. Confirm `/mcp` lists TeamShared tools.
4. `$status` (or `$teamshared-status`) — health + `version`
   (`installed_rule_version` 1.29.0).
...
| `skills/teamshared-memory/` | Protocol **1.29.0** (same loop as `rules/teamshared.mdc`) |
...
1. **Skill description** + `allow_implicit_invocation` so Codex loads the
   1.29.0 loop when TeamShared MCP tools are present.
...
3. **`SessionStart` `additionalContext`** — official Codex hook output that
   injects protocol 1.29.0 at session start (`startup`, `resume`, `clear`,
   `compact`).
...
| Event | What it does |
|---|---|
| `SessionStart` | Injects protocol 1.29.0 via `additionalContext`; maps Codex `session_id` → `memory_session_ensure` |
...
On the first turn, call `version` with `installed_rule_version` `1.29.0`.
