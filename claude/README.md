...
protocol **1.29.0**, SessionStart injection, and chat-capture hooks.
...
1. `/reload-plugins` (or start a new session) so `SessionStart` injects protocol 1.29.0.
2. Confirm `/mcp` lists TeamShared tools as connected.
3. `/teamshared:status` — health + `version` (`installed_rule_version` 1.29.0).
...
| Event | What it does |
|---|---|
| `SessionStart` | Injects protocol 1.29.0 via `additionalContext`; maps Claude `session_id` → `memory_session_ensure` |
