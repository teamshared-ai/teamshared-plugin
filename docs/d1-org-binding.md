...
**Follow-up (server):** MCP-native chat bind/switch is [teamshared#541](https://github.com/teamshared-ai/teamshared/issues/541) / [PR #542](https://github.com/teamshared-ai/teamshared/pull/542): `org_list` / `org_context_get` / `org_bind(slug, scope=conversation|workspace)` / `org_unbind`. Precedence: path `/o/{slug}/mcp` > conversation > workspace > OAuth/`tsk_`. CLI `teamshared org bind` remains an optional fallback.
